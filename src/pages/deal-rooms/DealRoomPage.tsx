...
export default function DealRoomPage() {
    ...
    // Find current group based on active view
    const currentGroup = groups.find(g => g.views.some(v => v.id === activeView));

    return (
        <div className="flex flex-col h-screen bg-background overflow-hidden">
            {/* Top Bar */}
            <header className="h-14 border-b border-border bg-card flex items-center justify-between px-3 md:px-4 shrink-0 z-20">
                <div className="flex items-center gap-2 md:gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold tracking-wider text-muted-foreground uppercase hidden sm:inline">DEAL ROOM ACTIVA</span>
                            <Badge variant="outline" className="flex md:hidden gap-1 py-0.5 px-2 bg-emerald-500/10 border-emerald-500/30">
                                <div className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                <span className="text-[10px] font-medium text-emerald-600">Activo</span>
                            </Badge>
                        </div>
                        <h1 className="text-sm font-semibold truncate max-w-[140px] xs:max-w-[200px] sm:max-w-[280px] md:max-w-md">{dealTitle}</h1>
                    </div>
                </div>

                {/* Groups Navigation - Desktop */}
                <div className="hidden md:flex items-center gap-1 bg-muted/30 rounded-lg p-1 border border-white/5">
                    {groups.map((group) => (
                        <Button
                            key={group.id}
                            variant="ghost"
                            ...
                            className={cn(
                                "h-8 px-4 rounded-md font-medium text-xs transition-all gap-2",
                    ...
                    <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                        ...
                        <SheetContent side="left" className="p-0 w-80">
                            <div className="p-4 border-b border-border bg-muted/30">
                                ...
                                <p className="text-xs text-muted-foreground mt-1">Secciones de la Deal Room</p>
                            </div>
                            <div className="py-4 overflow-y-auto max-h-[calc(100vh-120px)]">
                                {groups.map((group) => (
                                    <div key={group.id} className="mb-4">
                                        <div className="px-4 py-2 text-xs font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                            ...
                            {/* Project Info Summary in Mobile Menu */}
                            <div className="p-4 border-t border-border bg-muted/20">
                                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Estado del Proyecto</p>
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-muted-foreground">Fondos</span>
            ...
            {/* Groups Navigation - Mobile (Horizontal Scroll) */}
            <div className="md:hidden border-b border-border bg-card px-3 py-2 shrink-0 overflow-x-auto">
                <div className="flex items-center gap-2 min-w-max">
                    {groups.map((group) => (
                        <Button
                            ...
                            onClick={() => handleGroupChange(group.id)}
                            className={cn(
                                "h-9 px-4 rounded-lg font-medium text-xs transition-all gap-2 whitespace-nowrap",
                                currentGroup?.id === group.id
            ...
            {/* Sub-elements Navigation Bar */}
            <div className="h-11 md:h-12 border-b border-border bg-background/50 backdrop-blur-sm flex items-center px-3 md:px-6 shrink-0 gap-2 md:gap-4 overflow-x-auto">
                ...
                <div className="flex items-center gap-1 min-w-max">
                    {currentGroup?.views.map((item) => (
                        <Button
                            ...
                            className={cn(
                                "h-8 px-3 rounded-md font-medium text-xs transition-all gap-1.5 md:gap-2 whitespace-nowrap",
                                activeView === item.id
                                    ? "bg-primary/10 text-primary"
                                    : "text-muted-foreground hover:text-foreground hover:bg-muted"
                            )}
                        >
                            <item.icon className={cn("h-3.5 w-3.5", activeView === item.id ? "text-primary" : "")} />
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* Left Sidebar - Project Info (Desktop only) */}
                <aside className="w-[340px] shrink-0 hidden xl:block border-r border-border bg-card">
                    <DealRightSidebar />
                </aside>
